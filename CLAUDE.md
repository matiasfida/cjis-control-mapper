...
4. **Query de `check_event.py` demasiado pobre para retrieval (detectado corriendo `eval.py`):** `_describe_event_for_search` armaba queries tipo `"audit log event: login failed"` — muy cortas y genéricas. Contra el índice real, esto daba Hit@5 de solo 25% (5/20 en `eval_set.py`), porque matcheaba el boilerplate de "policy and procedures" (AU-1, página 67) que se repite casi textual al inicio de cada familia de control, en vez del control específico. Confirmado que el índice/embeddings no eran el problema: la misma búsqueda con frase más parecida a la prosa de la política (ej. `"unsuccessful login attempts account lockout"` en vez de `"audit log event: login failed"`) sí encontraba el control correcto (AC-7) en el top-3. Fix: se sacó el prefijo "audit log event:" y se agregó `_SYNONYM_EXPANSIONS` en `check_event.py` — expansión por palabra suelta (no acción completa, para no hardcodear la respuesta esperada por evento) hacia vocabulario de la prosa real. Hit@5 subió a 80% (16/20), MRR a 0.65. Cada expansión se validó contra el índice real antes de agregarla (algunas palabras probadas empeoraban el ranking y se descartaron).
...
Los 4 misses restantes de `eval.py` tras el fix de retrieval (AU-8, IR-6, SI-3, CM-6) caen en este mismo patrón: controles adyacentes o de la misma familia/página (ej. AU-7/AU-8 ambos en página 73), no un problema de vocabulario de búsqueda.
...
- [x] Mejorar `_describe_event_for_search` (`check_event.py`) — hecho. Se sacó el prefijo genérico "audit log event:" (matcheaba boilerplate de AU-1) y se agregó `_SYNONYM_EXPANSIONS`, expansión por palabra suelta (no acción completa, para no hardcodear la respuesta esperada) hacia vocabulario de la prosa real de la política. Hit@5 subió de 25% (5/20) a 80% (16/20), MRR de 0.11 a 0.65. Los 4 misses restantes (AU-8, IR-6, SI-3, CM-6) son casos borde entre controles adyacentes/misma página, consistentes con la limitación de precisión ya documentada abajo.
